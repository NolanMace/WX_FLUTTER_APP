...
class PoolLotteryRecordPane extends StatefulWidget {
  const PoolLotteryRecordPane({super.key});

  @override
  State<PoolLotteryRecordPane> createState() => _PoolLotteryRecordPaneState();
}

class _PoolLotteryRecordPaneState extends State<PoolLotteryRecordPane> {
  final Dio _dio = Dio();

  //网络请求相关参数
  final String _getPoolLotteryRecordAdmin = AppConfig.getPoolLotteryRecordAdmin;
  late List<dynamic> _recordsList;
  ...
  void fetchData() async {
    try {
      ...
      setState(() {
        ...
        _currentPage = 0;
        _recordsList = response.data;
        _searchResult = _recordsList;
  ...
  void _selectAppId() {
    ...
    if (keyword.isEmpty) {
      ...
      setState(() {
        _selectedItemIds.clear();
        _appIdResult = _recordsList.where((element) {
  ...
  void initState() {
    ...
    _columns = [
      ...
      const DataColumn(
        label: SizedBox(
          width: 60,
          child: Text("池子ID", style: TextStyle(fontSize: 12)),
        ),
      ),
      const DataColumn(
        label: SizedBox(
          width: 80,
          child: Text("池子名称", style: TextStyle(fontSize: 12)),
        ),
      ),
      const DataColumn(
        label: SizedBox(
                                    ...
                                    return DataRow(cells: [
                                      ...
                                      DataCell(
                                        SizedBox(
                                          width: 60,
                                          child: Text(
                                            _currentPageData[index]['pool_id']
                                                .toString(),
                                          ),
                                        ),
                                      ),
